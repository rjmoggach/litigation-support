...
export function useProfileData() {
    ...
    const loadPersonData = async () => {
        ...
        try {
            ...
            for (const marriage of marriagesArray) {
                try {
                    const childrenRes = await fetch(`/api/v1/marriages/${marriage.id}/children`, {
                        headers: getAuthHeaders()
                    })
                    if (childrenRes.ok) {
                        childrenData[marriage.id] = await childrenRes.json()
                    }
                } catch (error) {
    ...
    const handleAddChild = async (marriageId: number, childData: any) => {
        ...
        try {
            client.setConfig({ headers: getAuthHeaders() })
            const response = await fetch(`/api/v1/marriages/${marriageId}/children`, {
                method: 'POST',
                headers: {
                    ...getAuthHeaders(),
                    'Content-Type': 'application/json'
    ...
    const handleUpdateChild = async (marriageId: number, childId: number, childData: any) => {
        ...
        try {
            client.setConfig({ headers: getAuthHeaders() })
            const response = await fetch(`/api/v1/marriages/${marriageId}/children/${childId}`, {
                method: 'PUT',
                headers: {
                    ...getAuthHeaders(),
                    'Content-Type': 'application/json'
    ...
    const handleRemoveChild = async (marriageId: number, childId: number) => {
        ...
        try {
            client.setConfig({ headers: getAuthHeaders() })
            await fetch(`/api/v1/marriages/${marriageId}/children/${childId}`, {
                method: 'DELETE',
                headers: getAuthHeaders()
            })
            setMarriageChildren(prev => ({
