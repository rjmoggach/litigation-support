...
} from '@/components/admin/contacts/people/person-columns'
import { PersonEditDialog } from '@/components/admin/contacts/people/person-edit-dialog'
...
export default function PeoplePage() {
    ...
    const handlePersonSave = async (personId: number | null, data: any) => {
        ...
        try {
            ...
            toast.error(personId ? 'Failed to update person' : 'Failed to create person')
            throw error
        }
    }

    const handlePersonDelete = useCallback(async (person: PersonResponse) => {
        if (!session?.accessToken) return
    ...
    const columns = useMemo(() => createPersonColumns(
        handlePersonEdit,
        handlePersonDelete,
        people.length,
