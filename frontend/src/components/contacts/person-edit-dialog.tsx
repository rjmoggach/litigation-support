...
import type { Person, PersonProfile, Company, CompanyPersonAssociation } from '@/lib/api/contacts.types'
...
const personEditSchema = z.object({
    ...
    company_associations: z.array(z.object({
        company_id: z.number(),
...
interface PersonEditDialogProps {
    ...
    onAutoSave?: (personId: number | null, data: Partial<PersonEditFormData>) => Promise<void>
    isCreating?: boolean
...
export function PersonEditDialog({ 
    ...
    const [companyAssociations, setCompanyAssociations] = React.useState<Array<{
    ...
    React.useEffect(() => {
        if (isCreating) {
            ...
            setCompanyAssociations(existingAssociations.map(assoc => ({
    ...
    React.useEffect(() => {
        ...
        performAutoSave()
    }, [debouncedValues, onAutoSave, person, isCreating, form.formState.isDirty])
    ...
    const handleSubmit = async (data: PersonEditFormData) => {
        try {
            const submitData = {
                ...
                company_associations: companyAssociations.filter(assoc => assoc.company_id !== null)
            }
    ...
    const addExpertise = () => {
        if (!newExpertise.trim() || expertiseList.includes(newExpertise.trim())) return
    ...
    const addCompanyAssociation = () => {
        setCompanyAssociations(prev => [...prev, {
            company_id: null,
            role: '',
            start_date: '',
            end_date: '',
    ...
    const updateCompanyAssociation = (index: number, field: string, value: any) => {
        setCompanyAssociations(prev => prev.map((assoc, i) => 
                                ...
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            ...
                                            <FormDescription>
                                                A brief professional biography (max 2000 characters)
                                            </FormDescription>
                                            <FormMessage />
                                        </FormItem>
                                    )}
                                    ...
                                    <div className="flex items-center justify-between">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <Select onValueChange={setNewExpertise}>
                                                ...
                                                <SelectContent>
                                                    {EXPERTISE_SUGGESTIONS.filter(exp => !expertiseList.includes(exp)).map((expertise) => (
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        ...
                                        {expertiseList.length === 0 && (
                                            <p className="text-sm text-muted-foreground">
                                                No expertise areas added yet. Use the dropdown above to add some.
                                            </p>
                                        )}
                                    </div>
                                </div>
                                    ...
                                    <div className="space-y-4">
                                        {companyAssociations.map((association, index) => (
                                            <div key={index} className="p-4 border rounded-lg space-y-4">
                                                <div className="flex items-center justify-between">
                                                    <h5 className="font-medium text-sm">Company #{index + 1}</h5>
                                                    <Button
                                                        type="button"
                                                        variant="ghost"
                                                        size="sm"
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                    <div>
                                                        ...
                                                        <Select 
                                                            onValueChange={(value) => updateCompanyAssociation(index, 'company_id', parseInt(value))}
                                                            value={association.company_id?.toString() || ''}
                                                        >
                                                            <SelectTrigger>
                                                    ...
                                                    <div>
                                                        ...
                                                        <Input
                                                            ...
                                                            onChange={(e) => updateCompanyAssociation(index, 'role', e.target.value)}
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                    <div>
                                                        ...
                                                        <Input
                                                            ...
                                                            onChange={(e) => updateCompanyAssociation(index, 'start_date', e.target.value)}
                                                        />
                                                    </div>
                                                    ...
                                                    <div>
                                                        ...
                                                        <Input
                                                            ...
                                                            onChange={(e) => updateCompanyAssociation(index, 'end_date', e.target.value)}
                                                ...
                                                <div className="flex items-center space-x-2">
                                                    <Checkbox
                                                        checked={association.is_primary}
                                                        onCheckedChange={(checked) => updateCompanyAssociation(index, 'is_primary', checked)}
                                                    />
                                                    <FormLabel className="text-sm">Primary Association</FormLabel>
                                                </div>
                                            </div>
                                        ))}
                                        ...
                                        {companyAssociations.length === 0 && (
                                            <p className="text-sm text-muted-foreground text-center py-8">
                                                No company associations added yet. Click "Add Company" to create associations.
                                            </p>
                                        )}
                                    </div>
                                </div>
