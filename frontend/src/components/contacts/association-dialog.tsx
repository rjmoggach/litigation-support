...
export function AssociationDialog({
    ...
    const [associationForm, setAssociationForm] = useState({
        ...
        start_date: existingAssociation?.start_date
            ? new Date(existingAssociation.start_date)
            : (null as Date | null),
        end_date: existingAssociation?.end_date
            ? new Date(existingAssociation.end_date)
            : (null as Date | null),
        ...
        try {
            if (editMode && existingAssociation) {
                await updateAssociationApiV1ContactsAssociationsCompanyIdPersonIdPut(
                    {
                        ...
                        body: {
                            ...
                            start_date: associationForm.start_date
                                ? format(
                                      associationForm.start_date,
                                      'yyyy-MM-dd',
                                  )
                                : undefined,
                            end_date: associationForm.end_date
                                ? format(associationForm.end_date, 'yyyy-MM-dd')
                                : undefined,
                ...
                const requestBody = {
                    ...
                    role: associationForm.role || undefined,
                    start_date: associationForm.start_date
                        ? format(associationForm.start_date, 'yyyy-MM-dd')
                        : undefined,
                    end_date: associationForm.end_date
                        ? format(associationForm.end_date, 'yyyy-MM-dd')
                        : undefined,
            ...
            toast.error(
                `Failed to ${editMode ? 'update' : 'create'} association: ${errorMessage}`,
            )
        } finally {
                ...
                <div className="grid gap-3 py-4">
                    ...
                    <div className="flex items-center space-x-2">
                        ...
                        <Label htmlFor="is_primary">
                            Primary association (shown under name)
                        </Label>
                    </div>
                </div>
